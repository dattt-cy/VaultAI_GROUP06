...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  return (
    <div className="relative flex flex-col h-full overflow-hidden bg-base">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-surface flex-shrink-0 gap-2 min-w-0">
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <MessageSquare className="w-4 h-4 text-accent" />
          <span className="text-[13px] font-semibold text-text-primary hidden sm:block">Cuộc trò chuyện</span>
          <span className="badge bg-elevated text-text-muted border border-border text-[11px]">{messages.length} tin</span>
        </div>
        <div className="flex items-center gap-1.5 min-w-0 flex-1 justify-end">
          {checkedCount > 0 ? (
            <div
              className="flex items-center gap-1 px-2 py-1 rounded-full bg-accent/10 border border-accent/20 min-w-0 max-w-[180px] sm:max-w-[220px]"
            ...
            <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-warning/10 border border-warning/25 flex-shrink-0">
              <BookOpen className="w-3 h-3 text-warning flex-shrink-0" />
              <span className="text-[11px] text-warning font-semibold whitespace-nowrap">Chưa chọn nguồn</span>
            </div>
          )}
          {isGenerating && (
            <div className="flex items-center gap-1 flex-shrink-0">
              <Loader2 className="w-3.5 h-3.5 text-accent animate-spin-fast" />
              <span className="text-[11px] text-text-muted whitespace-nowrap hidden sm:block">Đang xử lý...</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div
        ...
        className="flex-1 overflow-y-auto px-2 sm:px-5 pt-3 sm:pt-5 pb-2 flex flex-col"
        role="log"
        aria-live="polite"
