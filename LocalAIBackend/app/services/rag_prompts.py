...
_HEADING_KEYWORDS = [
    "đề mục", "mục lớn", "mục nhỏ", "phần lớn", "phần nhỏ",
    "cấu trúc tài liệu", "bố cục tài liệu", "mục lục", "các phần",
    "các chương", "các mục", "tiêu đề", "heading", "table of contents",
    "nêu mục", "liệt kê mục", "các đề mục",
]

_HEADING_TOP_ONLY_KEYWORDS = [
    "chỉ đề mục lớn", "đề mục lớn thôi", "mục lớn thôi",
    "chỉ mục lớn", "chỉ chương lớn", "đề mục chính",
    "chỉ cần đề mục lớn", "đề mục cấp 1",
]
...
def is_heading_intent(query: str) -> bool:
    """Câu hỏi về cấu trúc/đề mục tài liệu — cần quét toàn bộ để liệt kê tiêu đề."""
    lower = query.lower()
    return any(k in lower for k in _HEADING_KEYWORDS)


def is_heading_top_only(query: str) -> bool:
    """Chỉ muốn đề mục lớn cấp 1, không cần mục con."""
    lower = query.lower()
    if any(k in lower for k in _HEADING_TOP_ONLY_KEYWORDS):
        return True
    # "đề mục lớn" mà không đề cập "nhỏ" → chỉ cần cấp 1
    has_large = "đề mục lớn" in lower or "mục lớn" in lower
    has_small = "nhỏ" in lower or "con" in lower or "cấp 2" in lower
    return has_large and not has_small
...
HEADING_PROMPT = PromptTemplate(
    ...
    template="""Bạn là chuyên gia phân tích cấu trúc tài liệu. Dưới đây là nội dung tài liệu.
...
Nhiệm vụ: Liệt kê TẤT CẢ đề mục lớn và đề mục nhỏ có trong tài liệu theo đúng cấu trúc phân cấp.
...
- TÊN CÔNG TY / TỔ CHỨC (ƯU TIÊN CAO): TUYỆT ĐỐI chỉ được nhắc tên công ty hoặc tổ chức nếu tên đó XUẤT HIỆN TRỰC TIẾP trong phần NGỮ CẢNH bên trên. Nếu NGỮ CẢNH không ghi tên công ty → chỉ viết "công ty", KHÔNG được lấy tên từ lịch sử hội thoại hoặc tự điền vào. Ví dụ sai: "Mức lương Senior tại công ty TechViet là..." khi NGỮ CẢNH không đề cập "TechViet". Ví dụ đúng: "Mức lương Senior là **25–40 triệu đồng/tháng**."
